...
export const AppearanceSettings = ({ appearanceData = {}, onSave, saving }) => {
  const [formData, setFormData] = useState({
    ...
    language: appearanceData.language || 'en',
  });
    ...
    <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-xs space-y-6">
      <div className="flex items-center justify-between pb-4 border-b border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-black text-slate-900">Appearance & Language</h3>
            <p className="text-xs font-semibold text-slate-500 mt-0.5">
              Select platform visual theme and display language settings
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="pt-4 border-t border-slate-100">
          ...
          >
            <option value="en">English (Official Government Administrative)</option>
          </select>
        </div>
